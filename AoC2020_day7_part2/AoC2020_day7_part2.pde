...
void setup() {
  ...
  int bags;
  bags=findBagRule("shiny gold bag",0);
  
  println("total bags:"+bags);
}
...
int findBagRule(String name, int depth)
{
  // Walk the list
  ...
  for (i=0;i<arraySize;i++)
  {
    ...
    if (temp.bagName.equals(name))
    {
      ...
      for (j=0;j<subrules;j++)
      {
        ...
        {
          //print("[D]="+depth);
          //bagCount+=temp.contentsCount[j];
          
          // count *these* bags, plus multiple for all the embedded bags.
          bagCount+=temp.contentsCount[j]+(temp.contentsCount[j] * findBagRule(temp.contentsName[j],depth+1));
        }
      }
    }
  }
